...
const AdminProfile = () => {
    ...
    const { genres, films, user } = useSelector(state => ({
        ...
        <div className="container">
            <div className="main">
                <div className="main-categories">
                    <h3>Жанры</h3>
                    {genres.map(genre => (
                        <Link key={genre.id} to={`/${genre.key}`}>{genre.name}</Link>
                    ))}
                </div>
                        ...
                        <div key={item.id} className="main-card">
                            <div className="main-card-left">
                                ...
                                <div className="main-film-info">
                                    <Link to={`/films/${item.id}`}>{item.titleRus}</Link>
                                    <p>{item.titleEng}, {item.year}, {item.time} мин</p>
                                    <p>{item.country?.name}, {item.genre?.name}</p>
                                    <p>Автор: {item.author?.fullName}</p>
                            ...
                            <div className="main-card-right">
                                ...
                                <div className="main-film-buttons ">
                                    ...
                                    <button className=" profile-info-exitButtoYYYYYYn" onClick={() => handleDelete(item.id)}>
                                        Удалить
                                    </button>
                                </div>
                            </div>
